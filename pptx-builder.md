...
| 检查项 | 标准 |
|-------|------|
| 视觉主心骨 | 每页是否有一个占据 1/3+ 版面的主视觉元素（数字/色块/图片/图标） |
| 元素对比 | 主要元素与次要文字的尺寸比是否 ≥ 3:1 |
| 内容密度 | 要点类页面是否 ≤ 6 条；Bento 是否 ≤ 6 张卡片 |
| 留白 | 内容是否未超版面 70%（避免塞满） |
| Vibe 一致性 | 极简震撼：是否有超大数字/色块；活力生态：是否有多卡片/图标 |
| 连续重复 | 是否连续超过 2 页使用同一版式 |
| **无边框卡片** | **卡片/格子禁止描边线（`line: { type: 'none' }`）；用阴影+色差区分层次** |
| **纯文本禁止** | **是否每张内容页都有视觉锚点（数字/图标/彩色面板/图片之一）** |
| **正文左对齐** | **段落/列表 `align` 不得为 `'center'`；标题同左（金蝶官方规范）** |
| **间距一致** | **同一张 PPT 只用 0.30" 或 0.50" 块间距，不混用** |
...
// ❌ 卡片加描边线
line: { color: 'DDDDDD', width: 0.5 }  →  line: { type: 'none' }（依赖阴影+色差区分）

// ❌ 纯文本内容页（无任何视觉元素）
→ 改用版式 27（双栏右视觉面板）或在版式 04 右侧补充图标块

// ❌ 段落/列表居中对齐
align: 'center' 用于正文  →  正文一律 align: 'left'（或不设，默认左对齐）
```
